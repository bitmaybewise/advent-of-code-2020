...
func (b boardingPass) reduce(s string, upperHalf, lowerHalf rune, endRange int) int {
	start, end := 0, endRange
	...
	return int(math.Max(float64(start), float64(end)))
}

func (b boardingPass) row() int {
	s := string(b)[0:7]
	return b.reduce(s, 'B', 'F', 127)
}

func (b boardingPass) column() int {
	s := string(b)[7:]
	return b.reduce(s, 'R', 'L', 7)
}
...
func parseBP(s []string) []boardingPass {
	bp := make([]boardingPass, len(s))
	for i, b := range s {
		bp[i] = boardingPass(b)
	}
	return bp
...
func answer1(b []boardingPass) int {
	var highest int
	for _, bp := range b {
...
func answer2(b []boardingPass) int {
	ids := make([]int, len(b))
	for i, bp := range b {
		ids[i] = bp.seatID()
	}
